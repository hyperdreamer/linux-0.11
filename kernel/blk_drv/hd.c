...
static void hd_out(unsigned int drive, unsigned int nsect, unsigned int sect,
                   ...
                   void (*intr_addr)(void))
{
	if (drive>1 || head>15) panic("Trying to write bad sector");
	if (!controller_ready()) panic("HD controller not ready");
    //////////////////////////////////////////////////////////////////////////
...
static void reset_controller(void)
{
    ...
    outb(4, HD_CMD);
    for(i = 0; i < 100; ++i) nop(); // wait a short while
    outb(hd_info[0].ctl & 0x0f, HD_CMD);
    /***************************************************************/
    if (drive_busy()) printk("HD-controller still busy\n\r");
    /***************************************************************/
    if ((i = inb(HD_ERROR)) != 1)
...
{
    if (++CURRENT->errors >= MAX_ERRORS) end_request(0);
    /***************************************************************/
...
void do_hd_request(void)
{
    ...
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; 
repeat:
    if (!init_request()) return;    // if init failed, return
    //////////////////////////////////////////////////////////////////////////
    ...
    __asm__ ("divl %4\n\t"
             :
             "=a" (block),  // the quotient
             "=d" (sec)     // the remainder
             :
             "0" (block),   // %eax: LS bits
             "1" (0),       // %edx: MS bits, here all 0
             "r" (hd_info[dev].sect)
            );
    /***************************************************************/
    __asm__ ("divl %4\n\t"
    ...
    if (CURRENT->cmd == WRITE) {
        ...
        for(int i = 0; i < 3000; ++i) {
            r = inb_p(HD_STATUS) & DRQ_STAT;
            if (r) break;   // if ready for next reqest
        }
        /*******************************************************/
        if (!r) {
            bad_rw_intr();
        ...
        /*******************************************************/
        port_write(HD_DATA, CURRENT->buffer, 256);  // 256*2 bytes
    } 
