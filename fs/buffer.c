...
struct buffer_head* hash_table[NR_HASH] = {};   // initialized to NULL
...
{
    if (MAJOR(dev) != 2) return;    // only for floppy disks
...
// ^: XOR, not power
#define _hashfn(dev, block) (((unsigned long) (dev^block)) % NR_HASH)
...
repeat:
    ...
    if (bh) return bh;
    // if bh == NULL, i.e. no buffer is found, read from the dev into buffer
...
struct buffer_head* bread(int dev, int block)
{
    ...
    ll_rw_block(READ, bh);  // low-level read/write block: (driver)
    wait_on_buffer(bh);

    if (bh->b_uptodate) return bh;

    brelse(bh);     // read failed, then release
    return NULL;
}
...
{
    void* b;
    if (buffer_end == 1<<20)    // if end at 1MB
        b = (void*) (640*1024); // neglet buffer_end
    else
        b = (void*) buffer_end;
    ///////////////////////////////////////////////////////////////////////////
    ...
    while ( (b -= BLOCK_SIZE) >= ((void *) (h+1)) ) {
        ...
        ++h;
        ++NR_BUFFERS;
        // check the boot procedure to see why 0xA0000 is used
        if (b == (void *) 0x100000) b = (void *) 0xA0000;
    }
    --h;                            // now h points to the end block
    free_list = start_buffer;       // set the header
    free_list->b_prev_free = h;     // circular linked list: point to the end
    h->b_next_free = free_list;     // circular linked list: point to the start
    /////////////////////////////////////////////////////////////////////////
    //for (int i = 0; i < NR_HASH; ++i) hash_table[i] = NULL;
}	
