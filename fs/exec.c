...
static unsigned long change_ldt(unsigned long text_size, unsigned long* page)
{
    ...
    unsigned long code_limit = (text_size + PAGE_SIZE -1) & 0xFFFFF000;
    unsigned long data_base = get_base(current->ldt[1]); // the same as code
    unsigned long data_limit = 0x4000000;     // 64 MB
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    for (int i = MAX_ARG_PAGES-1; i >= 0; --i) {
        data_base -= PAGE_SIZE;
              ...
              char** envp)
{
    ...
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    if ((0xffff & eip[1]) != 0x000f)    // if %cs != ldt[1]
...
restart_interp:
    ...
    /***************************************************************/
    i = inode->i_mode;
    int e_uid = (i & S_ISUID) ? inode->i_uid : current->euid;
    int e_gid = (i & S_ISGID) ? inode->i_gid : current->egid;
    //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if ((bh->b_data[0] == '#') && (bh->b_data[1] == '!') && (!sh_bang)) {
        ...
        //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        strncpy(buf, bh->b_data+2, 1022);   // skip '#!'
        buf[1022] = '\0';
        /***************************************************************/
        brelse(bh);
        iput(inode);
        ...
        argc++;
        /***************************************************************/
        if (!p) {   // get_free_page() failed! or too many args
            retval = -ENOMEM;
            goto exec_error1;
        }
        //////////////////////////////////////////////////////////////////////
        ...
        old_fs = get_fs();
        set_fs(get_ds());
        // NOTE!!! namei get filename from %fs
        inode = namei(interp);  // get the inode of the shell
        if (!inode) { /* get executables inode */
            set_fs(old_fs);
            retval = -ENOENT;
            goto exec_error1;
        }
    ...
    current->end_data = current->end_code + ex.a_data;
    current->brk = current->end_data + ex.a_bss;
    current->start_stack = p & 0xfffff000;  // at 4 KB boundary
    current->euid = e_uid;
    current->egid = e_gid;
    /***************************************************************/
    i = current->end_data;
    while (i & 0xfff) put_fs_byte(0, (char*) (i++)); // zero out 1 page in .bss
    /***************************************************************/
