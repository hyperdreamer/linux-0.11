...
struct super_block* get_super(int dev)
{
    ...
    for (s = &super_block[0]; s < &super_block[NR_SUPER]; ++s) {
        if (s->s_dev == dev) {
            ...
            s = &super_block[0];    // reset the searching process
        }
    }
    return NULL;
}
...
static struct super_block* read_super(int dev)
{
    if (!dev) return NULL;
    /////////////////////////////////////////////////////////////////////////
    check_disk_change(dev); // only for floppy disks, skip it now
    /////////////////////////////////////////////////////////////////////////
    struct super_block* s = get_super(dev);
    if (s) return s; // if dev's super block has already been loaded
    ...
    for (i = 0 ; i < nr_slots; ++i)
        if ((s->s_zmap[i] = bread(dev, block)))
            ...
            break;
    /////////////////////////////////////////////////////////////////////////
    // now `block' should be the nr of the first i-node block
